...
func (s *ChromedpStrategy) GetMasterPlaylist(ctx context.Context, url string) (string, string, string, error) {
	...
	// Channel to signal when m3u8 is found
	...
	chromedp.ListenTarget(ctx, func(ev interface{}) {
		if req, ok := ev.(*network.EventRequestWillBeSent); ok {
			...
			if strings.Contains(url, ".m3u8") {
				log.Info().Str("candidate_url", url).Msg("Found m3u8 candidate")
				...
				if !isAd {
					// If it looks like a manifest, take it immediately
					if isManifest {
						select {
						case foundCh <- url:
						default:
							// Already have one, but this is a manifest!
							// Ideally we'd replace the old one if the old one wasn't a manifest.
							// But for simplicity, let's assume the first manifest we see is the one.
						}
					} else {
						// If it's not explicitly a manifest, but not an ad, take it if we have nothing
						select {
						case foundCh <- url:
						default:
						}
	...
	err := chromedp.Run(ctx,
		chromedp.Navigate(url),
		chromedp.WaitVisible(`body`, chromedp.ByQuery),
		chromedp.Sleep(5*time.Second), // Wait for network requests
		chromedp.ActionFunc(func(ctx context.Context) error {
			var err error
			cookies, err = network.GetCookies().Do(ctx)
			return err
	...
	select {
	case m3u8URL = <-foundCh:
		// Found it
	default:
		// If not found in network, try to extract from video source or iframes?
		// But for Dailymotion, the m3u8 is usually fetched.
		// If failed, maybe we are blocked or page structure changed.
		return "", "", "", fmt.Errorf("m3u8 playlist not found in network traffic")
	}
	...
	return m3u8URL, cookieBuilder.String(), ua, nil
}
