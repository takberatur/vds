...
export const load = async ({ locals, url, parent }) => {
	...
	const title = await deps.languageHelper.singleTranslate(i18n.about_us(), lang) as SingleResponse;
	...
	const description = await deps.languageHelper.singleTranslate(i18n.about_us_description({ site_name: settings?.WEBSITE.site_name || 'Video Downloader' }), lang) as SingleResponse;
