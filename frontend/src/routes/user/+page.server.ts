	...
	const pageMetaTags = defaultMetaTags(
		{
			...
			title: `${i18n.update_profile()} - ${settings?.WEBSITE.site_name || 'Video Downloader'}`,
...
export const actions = {
	default: async ({ locals, request }) => {
		...
		const result = await locals.deps.userService.clientUpdateProfile(form.data);
		if (result instanceof Error) {
			return fail(500, {
				form,
				message: result.message || i18n.user_profile_update_error()
			});
		}
		...
		return {
			form,
			success: true,
			message: i18n.user_profile_update_success()
		};
	}
};
