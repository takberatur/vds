<script lang="ts">
	...
	import { AdminSidebarLayout, AdminDownloadTable } from '@/components/admin';
	import { DateRangeInput, AppAlertDialog, CardSpotlight } from '@/components/index.js';
	import { Button } from '$lib/components/ui/button/index.js';
	import Icon from '@iconify/svelte';
	import { updateUrlParams, createDownloadManager } from '@/stores/query.js';
	...
	let dataEntityAction = $state<Download | null | undefined>(null);

	const queryManager = createDownloadManager();
	...
	async function handleSingleDelete() {
		...
		try {
			const response = await fetch(`/download/${dataEntityAction.id}`, {
				method: 'DELETE'
			});

			const result = await response.json();
			if (!response.ok) throw new Error(result.message || 'Failed to delete download');
	...
	async function handleBulkDelete(ids: string[]) {
		...
		try {
			...
			const result = await response.json();
			if (!response.ok) throw new Error(result.message || 'Failed to delete downloads');
...
<MetaTags {...metaTags} />
<AdminSidebarLayout page="Download" user={data.user} setting={data.settings}>
	<div class="@container/main flex flex-col gap-4 md:gap-6">
		<div class="flex-none px-4 py-4 sm:px-6">
			<div class="space-y-1">
				<h1 class="text-2xl font-bold tracking-tight sm:text-3xl">Download</h1>
				<p class="text-sm text-muted-foreground">Manage your download user data.</p>
			</div>
		</div>
			...
			>
				<div class="overflow-hidden rounded-lg border bg-white/60 px-1 py-4 dark:bg-black/60">
					<AdminDownloadTable
						...
						onView={(val) => {
							if (val) goto(localizeHref(`/download/${val.id}`));
						}}
						onDelete={(download) => {
							if (download) {
	...
	<AppAlertDialog
		...
		type="warning"
		title="Warning"
		message={`Are you sure you want to delete ${dataEntityAction?.title || dataEntityAction?.id || ''}?`}
		labelClose="Cancel"
		labelAction="Confirm"
		onaction={() => {
			if (dataEntityAction) {
