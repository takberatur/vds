<script lang="ts">
	...
	const { form, errors, submitting, enhance } = superForm(data.form, {
		id: `update-application-form-${data.application.id}`,
		dataType: 'json',
		resetForm: false,
		onSubmit() {
...
<MetaTags {...metaTags} />
<AdminSidebarLayout
	page={`Edit Application - ${data.application.name}`}
	user={data.user}
...
>
	<div class="@container/main flex flex-col gap-4">
		<div class="flex-none px-4 py-4 sm:px-6">
			<div class="space-y-1">
				<h1 class="text-2xl font-bold tracking-tight sm:text-3xl">Edit Application</h1>
				<p class="text-sm text-muted-foreground">
					Edit application details for <span class="font-bold">{data.application.name}</span>.
				</p>
			</div>
		</div>
