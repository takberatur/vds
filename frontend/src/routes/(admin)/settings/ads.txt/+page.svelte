...
<MetaTags {...metaTags} />
<AdminSidebarLayout page="ads.txt Setting" user={data.user} setting={data.settings}>
	<AdminSettingLayout fixed title="ads.txt Setting" description="Update ads.txt setting">
		{#snippet children()}
			<form method="POST" class="min-h-0 flex-1 space-y-4 pb-14" use:enhance>
				<Field.Group>
					<Field.Set>
						<Field.Field>
							<Field.Label for="content">Ads.txt Content</Field.Label>
							<Textarea
