<script lang="ts">
	...
	async function handleSingleDelete() {
		...
		try {
			const response = await fetch(`/subscription/${dataEntityAction.id}`, {
				method: 'DELETE'
			});
	...
	async function handleBulkDelete(ids: string[]) {
		...
		try {
			const response = await fetch('/subscription/bulk', {
				method: 'DELETE',
				headers: {
			...
			const result = await response.json();
			if (!response.ok) throw new Error(result.message || 'Failed to delete subscriptions');
			toast.success('Subscription deleted successfully');
			await refresh();
		} catch (error) {
			toast.error(error instanceof Error ? error.message : 'Unknown error');
...
<MetaTags {...metaTags} />
<AdminSidebarLayout page="Subscription" user={data.user} setting={data.settings}>
	<div class="@container/main flex flex-col gap-4 md:gap-6">
		<div class="flex-none px-4 py-4 sm:px-6">
			<div class="space-y-1">
				<h1 class="text-2xl font-bold tracking-tight sm:text-3xl">Subscription</h1>
				<p class="text-sm text-muted-foreground">Manage your Subscription user data.</p>
			</div>
		</div>
	...
	<AppAlertDialog
		...
		type="warning"
		title="Warning"
		message={`Are you sure you want to delete subscription ${dataEntityAction?.product_id || 'N/A'} from user ${dataEntityAction?.user?.full_name || 'N/A'}?`}
		labelClose="Cancel"
		labelAction="Confirm"
		onaction={() => {
			if (dataEntityAction) {
