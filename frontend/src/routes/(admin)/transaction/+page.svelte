<script lang="ts">
	...
	let dataEntityAction = $state<Transaction | null | undefined>(null);
	...
	async function handleSingleDelete() {
		...
		try {
			const response = await fetch(`/transaction/${dataEntityAction.id}`, {
				method: 'DELETE'
			});

			const result = await response.json();
			if (!response.ok) throw new Error(result.message || 'Failed to delete transaction');
	...
	async function handleBulkDelete(ids: string[]) {
		...
		try {
			const response = await fetch('/transaction/bulk', {
				method: 'DELETE',
				headers: {
...
<MetaTags {...metaTags} />
<AdminSidebarLayout page="Transaction" user={data.user} setting={data.settings}>
	<div class="@container/main flex flex-col gap-4 md:gap-6">
		<div class="flex-none px-4 py-4 sm:px-6">
			<div class="space-y-1">
				<h1 class="text-2xl font-bold tracking-tight sm:text-3xl">Transaction</h1>
				<p class="text-sm text-muted-foreground">Manage your Transaction user data.</p>
			</div>
		</div>
	...
	<AppAlertDialog
		...
		type="warning"
		title="Warning"
		message={`Are you sure you want to delete transaction ${dataEntityAction?.id || 'N/A'} from user ${dataEntityAction?.user?.full_name || 'N/A'}?`}
		labelClose="Cancel"
		labelAction="Confirm"
		onaction={() => {
			if (dataEntityAction) {
